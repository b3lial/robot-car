...
// Early-stop: stop BLE scan as soon as this BDA is seen
static uint8_t s_scan_stop_bda[6];
static bool s_scan_stop_on_bda = false;
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    if (s_scan_stop_on_bda && memcmp(scan_rst->bda, s_scan_stop_bda, 6) == 0) {
        s_scan_stop_on_bda = false;
        // Add to results even if HID filter didn't match — we know it's a HID device from NVS
        if (find_scan_result(scan_rst->bda, ble_scan_results) == NULL) {
            add_ble_scan_result(scan_rst->bda, scan_rst->ble_addr_type, appearance, adv_name, adv_name_len, scan_rst->rssi);
        }
...
esp_err_t esp_hid_ble_scan(uint32_t seconds, const uint8_t *stop_on_bda, size_t *num_results, esp_hid_scan_result_t **results)
{
    ...
    if (stop_on_bda) {
        memcpy(s_scan_stop_bda, stop_on_bda, 6);
        s_scan_stop_on_bda = true;
    }
